...
| 2026-06-15 | v1.5.1 | Safe Mode (strobe gate Option C) shipped in v1.5.1 via NULL_ANALYSIS frame substitution | Zeroing all audio reactivity bands (NULL_ANALYSIS) was the correct gate — strobeGate=0 blacks the canvas by shader contract; band-capping at 0.4–0.6 still strobed. Full implementation across web.html, mobile.html, output.html with topbar indicator dot and localStorage persistence | Joe | FROZEN ✓ validated |
| 2026-06-15 | v1.5.1 | `ology_strobe` localStorage key: '1'=Safe Mode on, '0'=off; absent=on (default) | Key name and value contract locked. Default-on protects photosensitive users on cold load | Joe | FROZEN ✓ validated |
